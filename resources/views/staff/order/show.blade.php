...
<div class="card">
  <div class="card-body">
    ...
    <td>
            ...
            <td class = "d-flex">
                <a href="{{route('staff.order.edit',$order->id)}}" class="btn btn-primary btn-sm float-left mr-1" style="height:30px; width:30px;border-radius:50%" data-toggle="tooltip" title="edit" data-placement="bottom"><i class="fas fa-edit"></i></a>
                <form method="POST" action="{{route('staff.order.destroy',[$order->id])}}">
                  @csrf
                  @method('delete')
...
<table class="table table-bordered">
    <thead>
        <tr>
            <th>Product</th>
            <th>Size</th> <!-- Add this line for the size column -->
            <th>Quantity</th>
            <th>Price</th>
        </tr>
    </thead>
    <tbody>
        @foreach ($order->cart as $cartItem)
            <tr>
                ...
                <td>{{ $cartItem->size }}</td> <!-- Display the size of the product -->
